...
| `core:data` | 171 | Repositories and `SmsIngressUseCase` (ingress ordering, override-preserving prune); `SettingsRepository`/`DraftRepository` defaults, stored key names and failure fallbacks |
...
| `feature:thread` | 80 | `ThreadViewModel`, SMS segment counting, emoji insertion + the thread and new-conversation screens |
| `feature:settings` | 86 | `SettingsViewModel`, `SpamSettingsViewModel`, `GeneralSettingsViewModel` + the settings pages and dialogs |
