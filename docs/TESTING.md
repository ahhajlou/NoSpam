...
| `core:data` | 194 | Repositories and `SmsIngressUseCase` (ingress ordering, override-preserving prune); `SettingsRepository`/`DraftRepository` defaults, stored key names and failure fallbacks |
...
| `feature:thread` | 102 | `ThreadViewModel`, SMS segment counting, emoji insertion + the thread and new-conversation screens |
| `feature:settings` | 99 | `SettingsViewModel`, `SpamSettingsViewModel`, `GeneralSettingsViewModel` + the settings pages and dialogs |
