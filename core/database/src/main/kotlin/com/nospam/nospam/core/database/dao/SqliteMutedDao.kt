...
class SqliteMutedDao(private val helper: SqliteNoSpamOpenHelper) : MutedDao {
    ...
    override suspend fun mute(threadId: Long) = withContext(Dispatchers.IO){ val v = android.content.ContentValues().apply{ put("threadId", threadId)}; helper.writableDatabase.insertWithOnConflict("muted_threads", null, v, android.database.sqlite.SQLiteDatabase.CONFLICT_IGNORE); flow.value = readAllSync() }
