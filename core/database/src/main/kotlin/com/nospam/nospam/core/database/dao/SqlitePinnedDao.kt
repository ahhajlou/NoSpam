...
class SqlitePinnedDao(private val helper: SqliteNoSpamOpenHelper) : PinnedDao {
    ...
    override suspend fun pin(threadId: Long) = withContext(Dispatchers.IO){ val v = android.content.ContentValues().apply{ put("threadId", threadId)}; helper.writableDatabase.insertWithOnConflict("pinned_threads", null, v, android.database.sqlite.SQLiteDatabase.CONFLICT_IGNORE); flow.value = readAllSync(); initialized.set(true) }
